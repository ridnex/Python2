...
result = StringVar()
output = Entry(root, textvariable = result, justify = RIGHT, font = 20, width = 20)
output.grid(row = 0, column = 0, columnspan = 4)
plus = lambda x: output.insert(END,str(x))
dele = lambda : output.delete(output.index(END)-1)
ac = lambda: output.delete(0, END)
def out():
    try:
        result.set(str(eval(output.get())))
    except:
        result.set("err")
